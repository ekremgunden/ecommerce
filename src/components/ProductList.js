...
const Product = ({data,categories}) => {
    ...
    console.log("CART" + cart.BasketReducer)
        ...
        <div className="flex flex-col">
            ...
            <div className="pt-8 flex flex-wrap product">
                    {data.filter(data => data.categories === active || active === 0).map(filteredData => (
                        <div key={filteredData.id} onMouseOver={()=>setDetailid(filteredData.id)} className="p-4 m-6 bg-white rounded shadow-md">
                            <Link to={`product-${filteredData.id}`} onClick={()=>handleBasket({ type: "CHANGE", payload: filteredData.id })} >
                                <div>
                                ...
                                <div>
                                    <button className="bg-blue-500 p-1 rounded-md" onClick={() => handleBasket({ type: "ADD", payload: filteredData },{type: "INCREASE", payload: filteredData.id})}>Sepete Ekle</button>
                                </div> 
                    </div>
                    ))}
                    <Toaster />
